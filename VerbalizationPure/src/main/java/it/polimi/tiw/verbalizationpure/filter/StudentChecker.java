package it.polimi.tiw.verbalizationpure.filter;
...
public class StudentChecker extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		System.out.println("Student filter running...");
		HttpSession s = req.getSession();
		if (s.isNew() || s.getAttribute("user") == null || ((User)s.getAttribute("user")).isTeacher()) {
			System.out.println("The user is null or not a student");
