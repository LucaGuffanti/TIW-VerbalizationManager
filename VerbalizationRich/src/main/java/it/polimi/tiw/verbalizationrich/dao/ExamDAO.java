...
public class ExamDAO {
	...
	public List<User> getAllSubscribersOfExamOrderByGrade(int courseId, Date examDate) throws SQLException {
		String query = "SELECT userID, name, surname, email, studentDegree, grade, gradeStatus FROM user AS U JOIN subscription AS S ON U.userID = S.studentId WHERE courseId=? AND examDate=? ORDER BY grade ASC";
		try (PreparedStatement p = connection.prepareStatement(query)) {
			p.setInt(1, courseId);
	...
	public int retrieveExamVerbalId(int courseId, Date examDate) throws SQLException {
		String query = "SELECT id FROM verbal WHERE courseId=? AND examDate=?";
		try (PreparedStatement p = connection.prepareStatement(query)) {
			p.setInt(1, courseId);
			p.setDate(2, examDate);
			try (ResultSet result = p.executeQuery()) {
    ...
    public void insertMultiGrades(int courseId, Date examDate, ArrayList<Integer> studentIds, String grade) throws SQLException {
		String query = "UPDATE subscription SET grade=?, gradeStatus='INSERITO' WHERE courseId=? AND examDate=? AND studentId=?";
