...
@WebServlet("/PresentTaughtExams")
public class PresentTaughtExams extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int chosenCourse = 0;
        if (!ParameterChecker.checkString(chosenCourseIdAsString)) {
        ...
        try {
            if (!courseDAO.checkCourseExistenceAndTeaching(chosenCourse, teacher.getId())) {
                response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                response.getWriter().print("You're trying to see exams for a course that doesn't exist or that you don't teach");
                return;
            }
        } catch(SQLException e) {
        ...
        try {
            ...
            subscribedExams = examDAO.getAllDatesFromcourseId(chosenCourse);
        }
        catch (SQLException e) {
            response.setStatus(HttpServletResponse.SC_BAD_GATEWAY);
            response.getWriter().print("Something went wrong with the database");
